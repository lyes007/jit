...
import { getMachines, getArticles } from '@/lib/queries';
...
export async function GET() {
  try {
    ...
    const [machines, articles] = await Promise.all([
      getMachines(),
      getArticles(),
    ]);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch filter data', message: error?.message || 'Unknown error' },
      { status: 500 }
    );
  }
}
